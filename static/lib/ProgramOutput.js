...
export default class ProgramOutput {
  constructor(ctx, autoTake) {
    ...
    ctx.events.add(new Context(), 'forceChange', () => {
      this.show.reload();
    });
  }
  showChanged(layers) {
  ...
  draw() {
    ...
    if (this.pendingLayers) {
      for (let i = 0; i < this.pendingLayers.length; i++) {
        const layer = this.pendingLayers[i];
        if (layer.instance.handlesOwnTransition && this.layers[i].instance.handlesOwnTransition) {
          if (!layer.instance.transitionFrom)
            layer.instance.setTransitionFrom(this.layers[i].instance);
          layer.instance.transitionProgress = 1 - this.fade;
          if (layer.checkReady())
            this.layers[i] = layer;
        }
      }
