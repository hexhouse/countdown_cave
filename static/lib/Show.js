...
class ShowLayer {
  constructor(ctx, config, old) {
    ...
    if (false && old) {
      console.log(JSON.stringify(old.config.params));
      if (JSON.stringify(old.config.params) == JSON.stringify(config.params)) {
        console.log('same', old.config.params, config.params);
        config.params = { ...old.config.params };
      } else {
        console.log('diff', old.config.params, config.params);
      }
    }

    if (config.params)
      Object.setPrototypeOf(config.params, ctx.params);
  
    const childCtx = Object.assign(Object.create(ctx), {
      config,
      globalTextures: ctx.textures,
      params: config.params || ctx.params,
      textures: Object.assign(Object.create(ctx.textures), {
        self: () => {
  ...
  draw(alwaysDraw) {
    ...
    if (this.usesLast) {
      this.getLastbuffer().drawInto(() => {
        // HAX
        if (lastLayer.config.if)
          lastLayer.config.if(lastLayer.ctx);
...
export default class Show {
  ...
  instantiateLayer(config, old) {
    return new ShowLayer(Object.assign(Object.create(this.ctx), {
      instantiateLayer: this.instantiateLayer.bind(this),
    }), config, old);
  }
