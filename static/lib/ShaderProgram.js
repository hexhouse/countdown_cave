class Shader {
  constructor(gl, path, shader, onchange, preprocess) {
    this.gl = gl;
    this.path = path;
    this.extraPreprocess = preprocess || (t => t);
    this.shader = shader;
    this.images = {};
  ...
  async reload() {
    const { gl, path, preprocess, shader } = this;
  ...
  async preprocess(textIn, url) {
    ...
    const directives = {
      pragma: name => {
        if (name == 'fancy_easing') {
          this.fancyEasing = true;
          return `uniform float transitionProgress;`;
        } else {
          return `#pragma ${name}`;
        }
        return '';
    ...
    parts.push(textIn);
    return this.extraPreprocess((await Promise.all(parts)).join(''));
  }
...
export default class ShaderProgram {
  ...
  get handlesOwnTransition() {
    return this.fs.fancyEasing == true;
  }
  setTransitionFrom(oldInstance) {
    const { gl } = this;

    if (this.vs.text != oldInstance.vs.text)
      return false;

    this.transitionFrom = oldInstance;
    this.fs = new Shader(
      gl, this.fsPath,
  ...
  buildUniforms() {
    ...
    for (const k in this.uniforms) {
      ...
      const info = gl.getActiveUniforms(program, [index], gl.UNIFORM_TYPE)[0];
      if (!loc) {
